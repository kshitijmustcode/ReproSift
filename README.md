...
**Implementation checklist:** [34-step execution plan](docs/execution-plan.md). Track completed steps and evidence there. Steps 1–19 are complete. Next: Step 20, investigation workspace.
...
The dashboard and demo store each have four runnable routes. Follow the [local development guide](docs/local-development.md) to start them with FastAPI, install Playwright Chromium, verify the MCP connection card, and view a fresh cart screenshot in the sample workspace. `pnpm test:python` builds the browser MCP server and verifies the Python boundary contracts. See [MCP setup](packages/mcp-server/README.md) and [backend setup](services/backend/README.md). The demo store provides the seeded SAVE10 cart fixture from [the sample case](docs/sample-case.md); it has no persistence or trusted reset/harness yet. Run `pnpm check` before accepting changes. Next: Step 13, add browser actions.
