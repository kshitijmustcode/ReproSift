...
Step 9 adds `GET /mcp/status` as a dashboard diagnostic. It returns `200 { status: "connected", mcp: <validated MCP status> }` or `503 { status: "unavailable", error: { code: "MCP_UNAVAILABLE", safeMessage, phase } }`. Step 12 adds `GET /browser/sample-cart/screenshot`, which returns a validated ephemeral PNG payload or a safe `503 BROWSER_UNAVAILABLE` lifecycle error. Step 14 adds an MCP-only evidence response with an opaque artifact ID, screenshot, action log, console output, and same-origin request method/path metadata; it is not persistent storage. Neither route redefines `/health`; API liveness does not imply MCP or browser readiness. The Next.js server validates these external responses before rendering; browser clients never call the MCP stdio process.

Step 15 adds `reset_browser_session(sessionId, path)` to the TypeScript MCP contract. It is idempotent for the stateless demo fixture: it clears browser cookies plus local/session storage and reloads the allowed seeded route. Each MCP-created Playwright context starts empty; a 12-command cap applies to navigation, reset, click, fill, and select operations. The demo cart has no backend state to reset until persistence is introduced.
...
Step 17 persists immutable `(sourceName, version)` requirement documents and their ordered heading chunks. Retrieval returns a bounded citation containing document ID/version, chunk ID, source name, heading, content, and score. The current local lexical ranker is a deterministic development baseline; the Step 18 provider adapter will add embeddings and PostgreSQL pgvector ranking while preserving this citation shape.
