...
The Chromium installation is a local development prerequisite, not a committed artifact. Step 13 adds bounded visible-text inspection plus exact role/name, label, and test-id click/fill/select actions. Step 14 adds an in-memory evidence collection: an opaque artifact ID, PNG screenshot, action log, console messages, and same-origin request metadata. Step 15 resets an owned context by clearing cookies and browser storage before reloading the seeded route, and caps each session at 12 mutating commands. Durable artifact retention belongs to Step 16.
