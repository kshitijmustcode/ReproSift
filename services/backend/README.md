...
`GET /health` returns `{"status":"ok","service":"reprosift-api"}`. This is process liveness only; it does not check a database, MCP connection or model provider. No external service credentials are needed. Step 16 adds SQLAlchemy models, Alembic migrations, and repositories for investigation history; Step 17 adds versioned Markdown requirement ingestion and source-backed lexical retrieval. API routes and workers are not implemented yet. Local development defaults to SQLite, while `DATABASE_URL` accepts the PostgreSQL deployment URL.
